...
import com.cz.widget.recyclerview.layoutmanager.widget.AbsCycleLayout.OnPageChangeCallback;

import java.lang.annotation.Retention;
import java.util.Locale;

import static com.cz.widget.recyclerview.layoutmanager.widget.AbsCycleLayout.SCROLL_STATE_DRAGGING;
import static com.cz.widget.recyclerview.layoutmanager.widget.AbsCycleLayout.SCROLL_STATE_IDLE;
import static com.cz.widget.recyclerview.layoutmanager.widget.AbsCycleLayout.SCROLL_STATE_SETTLING;
import static java.lang.annotation.RetentionPolicy.SOURCE;

/**
 * Translates {@link RecyclerView.OnScrollListener} events to {@link OnPageChangeCallback} events
 * for {@link AbsCycleLayout}. As part of this process, it keeps track of the current scroll position
...
public class ScrollEventAdapter extends RecyclerView.OnScrollListener {
    private static final String TAG="ScrollEventAdapter";
    /** @hide */
    @Retention(SOURCE)
    ...
    private AbsCycleLayout.OnPageChangeCallback mCallback;
    private final @NonNull
    AbsCycleLayout mViewPager;
    private final @NonNull RecyclerView mRecyclerView;
    private final @NonNull CenterLayoutManager mLayoutManager;

    // state related fields
    private @AdapterState int mAdapterState;
    private @AbsCycleLayout.ScrollState int mScrollState;
    ...
    public ScrollEventAdapter(@NonNull AbsCycleLayout viewPager) {
        mViewPager = viewPager;
        mRecyclerView = mViewPager.recyclerView;
        //noinspection ConstantConditions
        mLayoutManager = (CenterLayoutManager) mRecyclerView.getLayoutManager();
    ...
    public void onScrolled(@NonNull RecyclerView recyclerView, int dx, int dy) {
        ...
        updateScrollEventValues();
        int currentCenterPosition=mLayoutManager.findCurrentItemPosition();
        if(mScrollPosition!=currentCenterPosition){
            dispatchSelected(currentCenterPosition);
        }
    ...
    private void updateScrollEventValues() {
        ...
        if(mLayoutManager.isInfiniteCycle()){
            values.mPosition = mLayoutManager.findCurrentItemPosition();
        } else {
        ...
        OrientationHelper orientationHelper = mLayoutManager.getOrientationHelper();
        boolean isHorizontal = mLayoutManager.getOrientation() == CenterLayoutManager.HORIZONTAL;
        int start, sizePx;
        if (isHorizontal) {
            sizePx = decoratedWidth;
            int leftOffset = (orientationHelper.getEnd() - mLayoutManager.getDecoratedMeasuredWidth(firstVisibleView)) / 2;
            start = firstVisibleView.getLeft() - leftOffset - leftDecorations - mRecyclerView.getPaddingLeft();
            if (mViewPager.isRtl()) {
                start = -start;
            }
        } else {
            sizePx = decoratedHeight;
            int topOffset = (orientationHelper.getEnd() - mLayoutManager.getDecoratedMeasuredHeight(firstVisibleView)) / 2;
            start = firstVisibleView.getTop() - topOffset - topDecorations - mRecyclerView.getPaddingTop();
        }
        values.mOffsetPx = -start;
        //We only check the offset pixel in the non-infinite cycle.
        if(!mLayoutManager.isInfiniteCycle()){
    ...
    private void startDrag(boolean isFakeDrag) {
        ...
        if (mTarget != NO_POSITION) {
            ...
            // Update "drag start page" to reflect the page that CycleViewPager thinks it is at
            mDragStartPosition = mTarget;
            // Reset target because drags have no target until released
            mTarget = NO_POSITION;
        } else if (mDragStartPosition == NO_POSITION) {
            // CycleViewPager was at rest, set "drag start page" to current page
    ...
    /**
     * @return {@code true} if the CycleViewPager is being dragged. Returns {@code false} from the
     *         moment the CycleViewPager starts settling or goes idle.
     */
    boolean isDragging() {
     ...
     *         {@link AbsCycleLayout#endFakeDrag()} is called.
     */
     ...
     * the ViewPager is settling towards its {@link AbsCycleLayout#getCurrentItem() current item}, or
    ...
    private void dispatchStateChanged(@AbsCycleLayout.ScrollState int state) {
